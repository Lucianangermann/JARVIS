...
_INSTRUCTIONS = (
    ...
    "CRITICAL — follow this to avoid hitting the tool-call limit:\n"
    "1. Read the file ONCE. Extract ONLY the key content (Lernziele, headings).\n"
    "2. Write ONE SINGLE document with ALL content in ONE create_file call. "
    "Never split into one file per section — always one file with all sections.\n"
    "3. If the document is long, write it completely in ONE create_file call "
    "with the full content string (all sections in one go). Avoid create+edit "
    "chains — each requires a separate confirmation and wastes iterations.\n"
    "4. If a file read was truncated, work with what you have. Do NOT re-read.\n"
    "5. After creating a notes/outline file, read THAT small file for subsequent "
    "writing — never re-read the original large PDF."
)
